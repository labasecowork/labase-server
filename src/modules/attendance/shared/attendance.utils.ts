...
import { MARK_ORDER, MarkType, PolicyLite, DayScheduleLite, WorkMode, MarkFlags } from "./attendance.constants";
...
export function getWeekday(d: Date): 1|2|3|4|5|6|7 {
  const wd = d.getDay();
  return ((wd + 6) % 7) + 1 as 1|2|3|4|5|6|7; // 1..7
}

// combina date con la fecha actual
export function combineDateAndTime(baseDate: Date, timeOnly?: Date): Date | undefined {
  if (!timeOnly) return undefined;
  const out = new Date(baseDate);
  out.setHours(timeOnly.getHours(), timeOnly.getMinutes(), timeOnly.getSeconds(), 0);
  return out;
}
...
export function getNextMarkType(
  ...
  const order = expected_points === 4 ? MARK_ORDER : (["entry", "exit"] as MarkType[]);
...
export function calcWorkedMinutes(points: Array<{ mark_type: MarkType; mark_time: Date }>): number {
  if (!points?.length) return 0;

  const byType = Object.fromEntries(points.map(p => [p.mark_type, p.mark_time])) as Record<MarkType, Date | undefined>;
  const entry = byType.entry;
  const exit  = byType.exit;

  const diffMin = (a?: Date, b?: Date) => (a && b ? Math.max(0, (b.getTime() - a.getTime()) / 60000) : 0);
...
export function evaluateMarkFlags(
  ...
  const within = (target?: Date, earlyMin?: number, lateMin?: number, allowEarlyBefore = true) => {
  ...
  switch (mark_type) {
    case "entry": {
      const r = within(targetFrom(schedule.entry_time), policy.early_before_minutes, policy.grace_entry_minutes, true);
      in_schedule = r.ok; is_late = r.late; is_early = r.early && !r.ok;
      break;
    }
    case "lunch_out": {
      const r = within(targetFrom(schedule.lunch_out_time), policy.early_before_minutes, policy.exit_late_minutes, true);
      in_schedule = r.ok; is_late = r.late; is_early = r.early && !r.ok;
      break;
    }
    case "lunch_in": {
      const r = within(targetFrom(schedule.lunch_in_time), policy.early_before_minutes, policy.exit_late_minutes, true);
      in_schedule = r.ok; is_late = r.late; is_early = r.early && !r.ok;
      break;
    }
    case "exit": {
      const r = within(targetFrom(schedule.exit_time), 0, policy.exit_late_minutes, false);
      in_schedule = r.ok; is_late = r.late; is_early = !r.ok && r.early;
      break;
    }
  }
...
export function isDayComplete(marked: MarkType[], expected_points: 2 | 4): boolean {
