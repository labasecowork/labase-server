...
import type { CreatePaymentDTO, Currency } from "../../../../../shared/payments/providers/payment-provider.repository";
...
export class VisaCallbackService {
  ...
  async execute(dto: VisaCallbackDTO) {
    ...
    if (!dto.transactionToken) {
      throw new AppError("MISSING_TRANSACTION_TOKEN", HttpStatusCodes.BAD_REQUEST.code);
    }
    ...
    if (!reservationId || !userId) {
      throw new AppError("INTERNAL_PAYMENT_ERROR", HttpStatusCodes.INTERNAL_SERVER_ERROR.code);
    }
...
const providerDto: CreatePaymentDTO = {
  ...
  dataMap: DEFAULT_DATA_MAP,
    ...
    const mappedStatus = result.success ? PaymentStatus.APPROVED : PaymentStatus.FAILED;
